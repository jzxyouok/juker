...
public class BrokerDaoImpl extends GenericHibernateDao<Broker> implements
	...
	public List<Broker> search(String name, Page page, Long comid) {
		...
		for (Iterator iterator = list.iterator(); iterator.hasNext();) {
			...
			brokers.add(new Broker(new Long(((BigInteger) objects[0])
					.intValue()), (String) objects[1], (String) objects[2],
					((BigDecimal) objects[3]).intValue(),
					((BigDecimal) objects[4]).intValue(),
					((BigDecimal) objects[5]).intValue(), (String) objects[6],
					(String) objects[7]));
		}
